...
const LinearSearch = () => {
    ...
    // Push / Insert / Delete — separate fields per operation
    ...
    // Reset visualization whenever the array or search target changes
    useEffect(() => {
        setIsFound('');
        setIdx(0);
        setIsEqual(false);
    ...
    // =========================================================
    // Insert (overwrite at index, or grow if index === length)
    // =========================================================
    const arrayInsert = async () => {
        ...
        if (index === array.length) {
            setArray((prev) => [...prev, Number(insertValue)]);
        } else {
            setArray((prev) => prev.map((item, i) => (i === index ? Number(insertValue) : item)));
        }
    ...
    return (
        <div>
            <Helmet>
                ...
                <meta
                    name="description"
                    content="Watch how linear search checks unsorted lists sequentially element-by-element. Understand O(n) search time complexity."
                />
