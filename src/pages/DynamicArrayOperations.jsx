...
export default function DynamicArrayOperations() {
  ...
  // Which operation tab is active
  const [activeTab, setActiveTab] = useState("create");

  // Fall back to Create if the array gets cleared elsewhere
  useEffect(() => {
  ...
  const arrayPushOperation = () => {
    ...
    setArray((prevArray) => [...prevArray, Number(pushValue)]);
  ...
  const arrayInsert = async () => {
    ...
    // Dynamic array insertion: inserting at the end grows the array.
    if (index === array.length) {
      setArray((prevArray) => [...prevArray, Number(insertValue)]);
    } else {
      setArray((prevArray) =>
        prevArray.map((item, i) => (i === index ? Number(insertValue) : item))
      );
    }
  ...
  return (
    <div>
      <Helmet>
        <title>Dynamic Array Operations Simulator | DSA Simulator</title>
        <meta
          name="description"
      ...
      <details
        ...
        onToggle={(e) => handleToggle("dynamicArrayOp", e.target.open)}
        open={detailsState["dynamicArrayOp"] !== undefined ? detailsState["dynamicArrayOp"] : true}
      >
        ...
        <div className="px-4 pb-5 sm:px-5">
          ...
          <div className="rounded-lg border border-border bg-bg p-4">
            <div className="mb-3 flex items-center justify-between">
              ...
              <span className="rounded-full bg-accent/10 px-2.5 py-1 text-xs font-medium text-accent">
                {hasArray ? `${array.length} elements` : "No array yet"}
              </span>
            </div>
            ...
            {activeTab === "create" && (
              <div className="flex flex-col gap-3">
                <p className="text-xs leading-relaxed text-muted">
                  Create a new dynamic array with an initial size. Creating a new array replaces the current one.
                </p>
            ...
            {activeTab === "pushpop" && (
              <div className="flex flex-col gap-3">
                <p className="text-xs leading-relaxed text-muted">
                  Push adds an element to the end and grows the array. Pop removes the last element and shrinks it —
                  this is what makes it "dynamic" instead of fixed-size.
                </p>
                <div className="flex flex-col gap-2 sm:flex-row">
                  <input
                    type="number"
            ...
            {activeTab === "insert" && (
              <div className="flex flex-col gap-3">
                <p className="text-xs leading-relaxed text-muted">
                  Insert a value at a specific index. Inserting at the end (index {hasArray ? array.length : "n"})
                  grows the array instead of overwriting.
                  {hasArray && (
                    <>
                      {" "}
            ...
            {activeTab === "delete" && (
              <div className="flex flex-col gap-4">
                <p className="text-xs leading-relaxed text-muted">
                  Delete by index removes the slot entirely and shifts later elements left, shrinking the array.
                  Delete by value marks the first match as{" "}
                  <span className="font-medium text-ink">NULL</span> in place.
                </p>
                ...
                <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
                  ...
                  <button
                    type="button"
                    onClick={deleteMode === "index" ? removeItemAtIndex : removeByEle}
                ...
                <div className="flex items-center gap-3 border-t border-border pt-3">
                  <p className="flex-1 text-xs text-muted">Or remove the whole array and start over.</p>
          ...
          <div className="mt-5 overflow-hidden rounded-xl border border-border bg-bg">
            <div className="flex items-center justify-between border-b border-border px-4 py-3 sm:px-5">
              ...
              {hasArray && (
                <span className="rounded-full bg-accent/10 px-2.5 py-1 text-xs font-medium text-accent">Dynamic</span>
              )}
            </div>
