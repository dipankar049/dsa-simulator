...
export default function HomePage() {
  ...
  return (
    <div className="w-full min-h-[92vh] bg-theme-gradient text-ink flex flex-col justify-start">
      ...
      <div className="max-w-7xl mx-auto w-full grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-16 items-center mt-4 sm:mt-6 lg:mt-12">
        ...
        <div className="lg:col-span-7 flex flex-col space-y-5 sm:space-y-6 text-center lg:text-left items-center lg:items-start animate-fade-in">
        ...
        {/* Right Side: Live Bubble Sort Demo */}
        <div className="lg:col-span-5 w-full mt-4 lg:mt-0">
          <div className="bg-surface border border-border rounded-2xl shadow-xl overflow-hidden animate-fade-in mx-2 sm:mx-0">
            {/* Window chrome */}
            ...
            <div className="p-6 flex flex-col space-y-4">
              ...
              <div className="overflow-x-auto py-3 px-1 border border-dashed border-border rounded-xl bg-bg/40 flex justify-center">
                <div className="w-fit">
                  <div
                    ...
                    style={{ gridTemplateColumns: `repeat(${demoArray.length}, auto)` }}
                  >
                    {/* Index row */}
                    {demoArray.map((_, idx) => (
                      <div
                        key={`idx-${idx}`}
                        className="flex h-6 w-12 shrink-0 items-center justify-center text-[10px] font-mono text-ink-muted"
                      >
                        {idx}
                      </div>
                    ))}
                    ...
                    {demoArray.map((val, idx) => {
                      ...
                      let cellStyle = {};
                      let customClasses = "border-border bg-surface text-ink";

                      if (isComparing) {
                        cellStyle = {
                          backgroundColor: isGreater ? "rgb(var(--ch-swapping))" : "rgb(var(--ch-comparing))",
                          borderColor: isGreater ? "rgb(var(--ch-swapping))" : "rgb(var(--ch-comparing))",
                          color: isGreater ? "rgb(var(--ch-swapping-text))" : "rgb(var(--ch-comparing-text))",
                        };
                      ...
                      return (
                        <div
                          key={`cell-${idx}`}
                          style={cellStyle}
                          className={`arrayDiv h-10 w-12 shrink-0 border rounded-md flex items-center justify-center text-xs transition-all duration-300 mx-0.5 ${customClasses}`}
                        >
                          {val}
                        </div>
                      );
              ...
              <Link
                href="/bubble-sort"
                className="w-full py-2.5 px-4 text-center text-xs sm:text-sm font-semibold bg-accent text-white rounded-lg shadow-lg shadow-accent/15 hover:shadow-accent/30 hover:brightness-105 transition-all flex items-center justify-center gap-1.5 hover:scale-[1.01]"
              >
      ...
      <div className="max-w-7xl mx-auto w-full mt-16 lg:mt-24">
        <h2 className="text-xl sm:text-2xl font-bold text-center mb-8 text-ink">
          The Simulator Visual Language
        </h2>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            { label: "Comparing Elements", colorBg: "bg-comparing", border: "border-comparing", text: "Evaluating relationship or condition" },
            { label: "Swapping Elements",  colorBg: "bg-swapping",  border: "border-swapping",  text: "Exchanging memory slots/positions" },
            { label: "Sorted / Finished",  colorBg: "bg-sorted",    border: "border-sorted",    text: "Successfully organized elements" },
            { label: "Landmark / Pivot",   colorBg: "bg-pivot",     border: "border-pivot",     text: "Active pointer reference or pivot element" },
          ].map((legend, index) => (
            <div key={index} className="bg-surface border border-border rounded-xl p-4 flex flex-col space-y-2">
              <div className="flex items-center space-x-2">
                ...
                <span className="font-semibold text-xs sm:text-sm text-ink">{legend.label}</span>
              </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {[
            ...
            <Link
              key={index}
              href={item.path}
              className="bg-surface hover:bg-surface-elevated border border-border hover:border-accent/40 rounded-xl p-5 shadow-sm hover:shadow-md transition-all flex flex-col justify-between group hover:scale-[1.01]"
            >
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  <h3 className="text-base sm:text-lg font-bold text-ink group-hover:text-accent transition-colors flex items-center gap-1.5">
